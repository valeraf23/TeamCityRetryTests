...
namespace TeamCityRetryTests.TeamCity.Locators
{
  ...
  public class BuildLocator
  {
    ...
    public override string ToString()
    {
      ...
      if (Status.HasValue)
        locatorFields.Add("status:" + Status.Value);

      if (Personal.HasValue)
        locatorFields.Add("personal:" + Personal.Value);

      if (Canceled.HasValue)
        locatorFields.Add("canceled:" + Canceled.Value);

      if (Running.HasValue)
        locatorFields.Add("running:" + Running.Value);

      if (Pinned.HasValue)
        locatorFields.Add("pinned:" + Pinned.Value);

      if (MaxResults.HasValue)
        locatorFields.Add("count:" + MaxResults.Value);

      if (StartIndex.HasValue)
        locatorFields.Add("start:" + StartIndex.Value);
